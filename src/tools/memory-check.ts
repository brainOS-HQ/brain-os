...
import { ToolContext } from "../storage/adapter.js";
...
export async function checkMemory(ctx: ToolContext, entityId?: string): Promise<MemoryCheckResult> {
  const allEntitiesRaw = await ctx.storage.listEntities();
  const entities: Entity[] = entityId
    ? allEntitiesRaw.filter((e) => e.id === entityId)
    : allEntitiesRaw;
  ...
  // Check overdue decision reviews — respect entity scope if set
  const overdue_reviews = allDecisions
    .filter((d) => d.status === "active" && d.review_date <= todayStr && (!entityId || d.entity_id === entityId))
    .map((d) => ({
      decision_id: d.id,
