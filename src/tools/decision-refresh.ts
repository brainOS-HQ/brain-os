 ...
 */
export async function refreshDecision(input: RefreshInput, ctx: ToolContext): Promise<RefreshResult> {
  assertSafeId(input.decision_id, "decision_id");
  const decisions = (await ctx.storage.getDecisions()) || [];
  ...
  await ctx.storage.putDecisions(decision.entity_id, decisions);
