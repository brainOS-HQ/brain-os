...
function warnOnProviderMismatch(total: number, compatible: number, active: string): void {
  ...
  console.error(
    `[brain-os] Embeddings provider mismatch: ${lost} of ${total} stored vectors were ` +
    `generated by a different provider and are invisible to the active "${active}" provider. ` +
    `Re-run entity_update / decision_log on affected items to re-embed under the current provider.`
  );
}
...
async function saveEmbeddings(embeddings: StoredEmbedding[]): Promise<void> {
  ...
  // Atomic write: serialize to a tmp file, then rename. Prevents corruption
  // from concurrent writers or a mid-write crash leaving partial JSON behind.
  // Same-directory tmp guarantees rename() is atomic on POSIX.
...
export async function semanticRecall(
  ...
  // Warn once per session if the user switched providers and silently lost
  // access to prior embeddings. Vectors from different models are not
  // comparable, so we can't transparently merge them — but the user should
  // know they need to re-embed.
  warnOnProviderMismatch(all.length, compatible.length, provider.name);
