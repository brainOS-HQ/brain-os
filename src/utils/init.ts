...
const COMMANDS: CommandSpec[] = [
  ...
  { template: "start.md",    rel: "brain/start.md",      name: "/brain:start" },
  { template: "focus.md",    rel: "brain/focus.md",      name: "/brain:focus" },
  { template: "queue.md",    rel: "brain/queue.md",      name: "/brain:queue" },
...
function getHookInfo(): HookTemplates {
  ...
  return {
    ...
    sessionEnd: {
      templatePath: join(hooksDir, "brain-os-sessionend.py"),
      available: existsSync(join(hooksDir, "brain-os-sessionend.py")),
    },
  };
}
...
export async function initBrain(targetDir: string, options: InitOptions = {}): Promise<string> {
  ...
  if (hook.routingGuard.available || hook.precompact.available || hook.sessionEnd.available) {
    output.push("");
  ...
  if (hook.sessionEnd.available) {
    output.push("");
    output.push("  Session-end checkpoint — saves state when a session ends without a wrap (Claude Code):");
    output.push(`    ${hook.sessionEnd.templatePath}`);
    output.push("    Add to .claude/settings.local.json under hooks.SessionEnd");
    output.push("    /start will surface unconfirmed checkpoints next session for review.");
  }
