...
- **`wrap_check`** — read-only detection. Counts state-changing work accumulated since the last wrap (from the audit trail), grouped by tool and project, and returns `recommend_wrap`. Tier 1 of auto-wrap: an agent calls it when you signal you're stopping, or periodically in a long session, to offer a wrap before context is lost.
- **`wrap_auto`** — a non-interactive wrap for when you're not there to review. Applies **low-risk** fields (`next_move`, `open_questions`, `evidence_of_progress`) immediately, and **stages high-risk** changes (`status`, `mode`, `blocked`, decisions) as an unconfirmed record for `/start` to surface and you to confirm. Always logs a `session_wrapped` marker. Provenance model: nothing machine-made enters trusted state unseen, and nothing is lost.
- **SessionEnd hook** (`templates/hooks/brain-os-sessionend.py`) — opt-in. Saves a checkpoint when a session ends without a wrap (closed terminal, `/clear`) — the companion to the existing PreCompact hook. Self-contained (won't affect the precompact hook) and gated to skip empty sessions.
- **`/brain:start`** — session-start brief. Reconciles unconfirmed auto-wraps and checkpoints first, then surfaces what accumulated, what's blocking, and one concrete first action.
- **`/brain:queue`** — pipeline view: active step, blockers, overdue decisions, next moves across projects, and the release queue.
- **Alias exclusivity enforcement** — `entity_update` now rejects an alias already claimed by a different entity at write time, preventing cross-entity contamination.

### Changed

- **`/wrap`** now reconciles auto-wrap records (not just raw checkpoints) and documents auto-mode (`wrap_check` → `wrap_auto`) with per-client behavior: the full safety net on clients that lose context (compaction, terminal close), detect-and-suggest only on clients with a persistent session.

### Fixed

- `memory_commit` and `context_resolve` edge-case fixes surfaced during dogfooding.

### Tests

- 51 smoke tests (up from 49). New coverage: `wrap_check` unwrapped-count + boundary reset after a wrap marker; `wrap_auto` provenance (low-risk applied to the entity, high-risk staged as unconfirmed).

## [0.8.0] — 2026-06-04

> Minor release: **assumption-aware review triggers.** Brain OS now connects changed project evidence to decision assumptions — `/reconcile` surfaces scan-triggered invalidations as reopening signals, not violations. Plus proactive guard improvements, follow-through accountability, and the `/scorecard` weekly health check.

### Added

- **`decision_review` v2: scan-based invalidation** — pass `root_path` to run `project_evidence_scan` against the repo and auto-match each decision's `invalidate_if` conditions. Matched decisions move to the `changed` bucket with `triggered_invalidate_if` citing the exact scan evidence that fired. Keyword-based match — verify before applying.
- **`follow_through_alerts` on `focus_get`** — surfaces active entities that stated a `next_move` but haven't logged any update in 7+ days. First-class accountability signal, same tier as blockers.
- **`/scorecard`** — new weekly health check command. Scores Brain OS state out of 100 across six dimensions: memory hygiene, reasoning quality, reliability, actionability, onboarding, and self-propulsion.
- **`scripts/backfill-atlas-embeddings.mjs`** — one-off utility to re-embed decisions under a new provider without losing prior state.

### Changed

- **`decision_check`: conflict escalation for semantic-only hits** — a semantic-only rejected-facet hit now promotes to `conflict` when it clearly dominates the chosen-facet match (margin ≥ 0.08). Provider-agnostic relative comparison — works correctly at both local (MiniLM, caps ~0.49) and OpenAI embedding scales.
- **`/reconcile`** updated to supply `root_path` when available and surface scan-triggered items with matched evidence lines.
- **`wrap.md`**: Step 6b added — review debt check per entity before session close.

### Fixed

- **`decision_log`: rejects placeholder `proof_action`** — values like "Review in next session", "TBD", "Revisit" are now rejected at log time with a clear error. Placeholder proofs create review debt without testable outcomes.
- **`memory_check`: scope leak fixed** — scoped calls (`entity_id` provided) no longer bleed `overdue_reviews` from other entities.

### Tests

- 49 smoke tests (up from 39). New coverage: `memory_check` scope isolation, `decision_check` escalation contract, stub detection with legacy-data write pattern.
